...
namespace UsedCarDashboard.Controllers
{
    public class UsersController : ApiController
    {
        ...
        public string Post(LoginUsers login)
        {
            try
            {
                string query = @"DECLARE	@responseMessage nvarchar(250) EXEC	dbo.VerifyLogin @pLoginName = N'" + login.LoginName + "', @pPassword = N'" + login.PasswordHash + "', @pEmpType = N'" + login.EmpType + "', @responseMessage = @responseMessage OUTPUT SELECT	@responseMessage as N'@responseMessage'  ";
                DataTable table = new DataTable();
                using (var con = new SqlConnection(ConfigurationManager.ConnectionStrings["UsedCarDashboard"].ConnectionString))
                using (var cmd = new SqlCommand(query, con))
                using (var da = new SqlDataAdapter())
